...
def main(_):
  ...
  for pen_ind in range(1):
    penalty =  0.9
    cfr_solver = cfr_ma.CFRPlusSolver(game, strategy_player=2,penalty=penalty)

    for i in range(FLAGS.iterations):
      print(i)
      cfr_solver.evaluate_and_update_policy()
      if i % FLAGS.print_freq == 0:
        conv = exploitability.nash_conv(game, cfr_solver.average_policy(), has_strategy = True)
        print("Iteration {} exploitability {}".format(i, conv))
    
    a = cfr_solver.average_policy()
    with open('autobattler_plus_{}.pkl'.format(penalty), 'wb') as fp:
          pickle.dump(a, fp)

    with open('autobattler_solver_plus_{}.pkl'.format(penalty), 'wb') as fp:
