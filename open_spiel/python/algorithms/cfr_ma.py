...
class _CFRSolverBase(object):
  ...
  def __init__(self, game, alternating_updates, linear_averaging,
               regret_matching_plus, penalty,strategy_player):
    # pyformat: disable
    """Initializer.
    ...
    self._strategy_player = strategy_player
  ...
  def _compute_counterfactual_regret_for_player(self, state, policies,
                                                reach_probabilities, player, level = 0):
    ...
    if state.is_chance_node():
      ...
      for action, action_prob in state.chance_outcomes():
        ...
        state_value += action_prob * self._compute_counterfactual_regret_for_player(
            new_state, policies, new_reach_probabilities, player,level+1)
      return state_value
    ...
    penalty = False
    if player == self._strategy_player and current_player!=self._strategy_player:
      average_prob = 1.0/len(leg_ac)
      #max_prob = max()
      # for action in leg_ac:
      #   action_prob = info_state_policy.get(action, 0.)
      #   if abs(action_prob-average_prob)<0.0001:
      #     penalty = True
      #     break
      for action in leg_ac:
        action_prob = info_state_policy.get(action, 0.)
        #penalize strategies with guaranteed actions
        if abs(1-action_prob)<0.05:
          penalty = True
          break
    ...
    for action in state.legal_actions():
      ...
      ireg = self._compute_counterfactual_regret_for_player(
          new_state,
          policies=policies,
      ...
      if not penalty:
        ...
        child_utility = ireg * self._penalty
...
class CFRSolver(_CFRSolver):
  ...
  def __init__(self, game,strategy_player,penalty):
    super(CFRSolver, self).__init__(
        ...
        alternating_updates=True,
        linear_averaging=False, penalty = penalty,strategy_player=strategy_player)
